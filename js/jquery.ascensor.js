  ...
  (Plugin.prototype.init = function() {
    ...
    function has3d() {
      ...
      return (
        ...
        support3D !== undefined && support3D.length > 0 && "none" !== support3D
      );
    }
    function getCss(index, property) {
      ...
      if (
        ...
        "top" == property && (transformAxis = "translateY"),
          (css = {
    ...
    function resize() {
      (NW = node.width()),
        ...
        chocolate &&
          (node
            .stop()
            .scrollLeft(self.options.direction[floorActive][1] * NW)
            .scrollTop(self.options.direction[floorActive][0] * NH),
          nodeChildren.each(function(index) {
            var css = {
            ...
            self.supportTransform &&
              (css = {
                transform:
                  "translateX(" +
                  100 * self.options.direction[index][1] +
                  "%) translateY(" +
                  100 * self.options.direction[index][0] +
                  "%)"
              }),
              $(this).css(css);
          }));
    ...
    function generateFloorMap() {
        ...
        return (
          $.each(floorCollection, function() {
            ((("right" == direction || "down" == direction) &&
              this[axis] > floor[axis]) ||
              (("left" == direction || "up" == direction) &&
                this[axis] < floor[axis])) &&
              (!closest ||
                Math.abs(this[axis] - goal) < Math.abs(closest[axis] - goal)) &&
              (closest = this);
          }),
          closest && -1 !== directionArray.indexOf(closest)
            ? directionArray.indexOf(closest)
            : !1
        );
      }
      function getfurthestFloor(floor, floorCollection, axis) {
        ...
        return (
          $.each(floorCollection, function() {
            (!furthest ||
              Math.abs(this[axis] - goal) > Math.abs(furthest[axis] - goal)) &&
              (furthest = this);
          }),
          furthest && -1 !== directionArray.indexOf(furthest)
            ? directionArray.indexOf(furthest)
            : !1
        );
      }
      function getIncrementedFloor(floorCollection, axis) {
        ...
        return (
          $.each(floorCollection, function() {
            (!floor ||
              Math.abs(this[axis] - goal) > Math.abs(floor[axis] - goal)) &&
              (floor = this);
          }),
          floor && -1 !== directionArray.indexOf(floor)
            ? directionArray.indexOf(floor)
            : !1
        );
      }
      ...
      function getFloor(x, y, floorOne, floorTwo) {
        return floorOne[0] + x == floorTwo[0] && floorOne[1] + y == floorTwo[1]
          ? directionArray.indexOf(floorTwo)
          : !1;
      }
      function getFurtherFloorOnAxis(floorArray, axis) {
        ...
        return (
          jQuery.each(floorArray, function(index, directionArray) {
            (furtherFloor === !1 ||
              furtherFloor[axis] < directionArray[axis]) &&
      ...
      var directionArray = jQuery.grep(self.options.direction, function(
          directionArray,
          index
        ) {
          return node.children().length > index;
        }),
        approximateFurtherX = getFurtherFloorOnAxis(directionArray, 1),
        sameAxisXFurthest = getSameAxisFloor(approximateFurtherX, 1),
        furtherY = getFurtherFloorOnAxis(sameAxisXFurthest, 0),
        approximateFurtherY = getFurtherFloorOnAxis(directionArray, 0),
        sameAxisYFurthest = getSameAxisFloor(approximateFurtherY, 0),
        furtherX = getFurtherFloorOnAxis(sameAxisYFurthest, 1);
      ...
      var approximateClosestX = getClosestFloorOnAxis(directionArray, 1),
        sameAxisXClosest = getSameAxisFloor(approximateClosestX, 1),
        closestY = getClosestFloorOnAxis(sameAxisXClosest, 0),
        approximateClosestY = getClosestFloorOnAxis(directionArray, 0),
        sameAxisYClosest = getSameAxisFloor(approximateClosestY, 0),
        closestX = getClosestFloorOnAxis(sameAxisYClosest, 1);
      (floorMap.closest_x = directionArray.indexOf(closestX)),
        ...
        $.each(directionArray, function(index, floorItem) {
          var axisXfloor = jQuery.grep(directionArray, function(
              directionArray
            ) {
              var isOnSameAxis = directionArray[0] == floorItem[0],
                isCurrentFloor = floorItem == directionArray;
            ...
            directNextXAxis = jQuery.grep(directionArray, function(
              directionArray
            ) {
              var isOnSameAxis = directionArray[0] == floorItem[0] + 1;
              return isOnSameAxis;
            }),
            directPreviousXAxis = jQuery.grep(directionArray, function(
              directionArray
            ) {
              var isOnSameAxis = directionArray[0] == floorItem[0] - 1;
              return isOnSameAxis;
            }),
            directNextYAxis = jQuery.grep(directionArray, function(
              directionArray
            ) {
              var isOnSameAxis = directionArray[1] == floorItem[1] + 1;
              return isOnSameAxis;
            }),
            directPreviousYAxis = jQuery.grep(directionArray, function(
              directionArray
            ) {
              var isOnSameAxis = directionArray[1] == floorItem[1] - 1;
              return isOnSameAxis;
          ...
          (floorMap[index] = {
            ...
            $.each(directionArray, function(indexSecond, floorItemSecond) {
              floorMap[index].down === !1 &&
                (floorMap[index].down = getFloor(
                  1,
                  0,
                  floorItem,
                  floorItemSecond
                ...
                floorMap[index].up === !1 &&
                  (floorMap[index].up = getFloor(
                    -1,
                    0,
                    floorItem,
                    floorItemSecond
                  )),
                floorMap[index].right === !1 &&
                  (floorMap[index].right = getFloor(
                    0,
                    1,
                    floorItem,
                    floorItemSecond
                  )),
                floorMap[index].left === !1 &&
                  (floorMap[index].left = getFloor(
                    0,
                    -1,
                    floorItem,
                    floorItemSecond
    ...
    function handleDirection(direction) {
      if ("y" == self.options.direction) {
        ...
        if (floorMap[floorActive][direction] !== !1)
          targetId = floorMap[floorActive][direction];
        else if (
          self.options.jump === !0 &&
          floorMap[floorActive].closest[direction] !== !1
        )
          targetId = floorMap[floorActive].closest[direction];
        else if (self.options.loop === !0)
          targetId = floorMap[floorActive].furthest[direction];
        else if (
          "loop-x" != self.options.loop ||
          ("right" != direction && "left" != direction) ||
          floorMap[floorActive].furthest[direction] === !1
        )
          if (
            "loop-y" != self.options.loop ||
            ("down" != direction && "up" != direction) ||
            floorMap[floorActive].furthest[direction] === !1
          ) {
            if ("string" == typeof self.options.loop) {
              var correctYDirection =
                  ("down" == direction || "up" == direction) &&
                  "increment-y" == self.options.loop,
                correctXDirection =
                  ("right" == direction || "left" == direction) &&
                  "increment-x" == self.options.loop;
              /* if a increment is possible */
              if (floorMap[floorActive].increment[direction] !== !1)
                (correctYDirection ||
                  correctXDirection ||
                  "increment" == self.options.loop) &&
                ...
                floorActive == floorMap.furthest_x
                  ? (targetId = floorMap.closest_x)
                  : floorActive == floorMap.closest_x &&
                    (targetId = floorMap.furthest_x);
              } else if ("down" == direction || "up" == direction) {
                if ("increment-x" == self.options.loop) return;
                floorActive == floorMap.furthest_y
                  ? (targetId = floorMap.closest_y)
                  : floorActive == floorMap.closest_y &&
                    (targetId = floorMap.furthest_y);
              }
            }
          } else targetId = floorMap[floorActive].furthest[direction];
        else targetId = floorMap[floorActive].furthest[direction];
        "number" == typeof targetId &&
    ...
    function getFloorFromHash() {
      if (window.location.hash) {
        ...
        var floor = !1;
        return (
          self.options.ascensorFloorName &&
            $.each(self.options.ascensorFloorName, function(index) {
    ...
    function scrollToStage(floor, time, firstrun) {
      ...
      if ("y" === self.options.direction)
        ...
        var sameXposition =
            node.scrollLeft() === self.options.direction[floor][1] * NW,
          sameYposition =
            node.scrollTop() === self.options.direction[floor][0] * NH;
        "x" === self.options.queued
          ? sameXposition
            ? (animationParams.property = {
                scrollTop: self.options.direction[floor][0] * NH
              })
      ...
      node
        ...
        firstrun &&
          "function" == typeof self.options.ready &&
          self.options.ready(),
        self.options.ascensorFloorName &&
          window.location.replace(
            ("" + window.location).split("#")[0] +
              "#" +
              self.options.ascensorFloorName[floor]
          ),
        (floorActive = floor),
    ...
    var //height/width settings
      ...
      self = this,
      node = $(this.element),
      nodeChildren = node.children(self.options.childType), //floor counter settings
      floorActive = self.options.windowsOn,
      floorCounter = -1,
      $document = (self.options.direction, $(document)),
      $window = $(window),
      chocolate = "object" == typeof self.options.direction;
    ...
    if (
      ...
      node.on("refresh", function() {
        (node.children().length > nodeChildren.length ||
          node.children().length < nodeChildren.length) &&
      ...
      (NH = node.width()),
      (NW = node.height()),
      chocolate && generateFloorMap(),
      node.data("current-floor", floorActive),
      self.options.keyNavigation && $document.keydown(checkKey),
      self.options.ascensorFloorName && window.location.hash)
    ) {
      var hashFloor = getFloorFromHash();
      hashFloor && (floorActive = hashFloor);
    }
    self.options.touchSwipeIntegration &&
      node.swipe({

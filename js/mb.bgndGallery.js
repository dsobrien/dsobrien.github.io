...
jQuery.fn.CSSAnimate = function(a, f, k, m, e) {
  return this.each(function() {
    ...
    if (0 !== b.length && a) {
      ...
      "function" == typeof m &&
        ((e = m), (m = "cubic-bezier(0.65,0.03,0.36,0.72)"));
      if ("string" == typeof f)
      ...
      if (jQuery.support.transition) {
        ...
        jQuery.browser.webkit
          ? ((d = "-webkit-"), (j = "webkitTransitionEnd"))
          : jQuery.browser.mozilla
            ? ((d = "-moz-"), (j = "transitionend"))
            : jQuery.browser.opera
              ? ((d = "-o-"), (j = "otransitionend"))
              : jQuery.browser.msie && ((d = "-ms-"), (j = "msTransitionEnd"));
        l = [];
        for (c in a) {
          var g = c;
          "transform" === g &&
            ((g = d + "transform"), (a[g] = a[c]), delete a[c]);
          "transform-origin" === g &&
            ((g = d + "transform-origin"), (a[g] = a[c]), delete a[c]);
          l.push(g);
        }
        var c = l.join(","),
          n = function() {
...
(function($) {
  $.mbBgndGallery = {
    ...
    buildGallery: function(options) {
      ...
      if (el.opt.onStart) el.opt.onStart();

      el.opt.gallery = $("<div/>")
        .attr({ id: "bgndGallery_" + el.opt.galleryID })
      ...
      if (el.opt.shuffle) el.opt.images = $.shuffle(el.opt.images);
    ...
    checkSize: function(image, el) {
      if ($.mbBgndGallery.changing) return;
      ...
      return image.each(function() {
        ...
        var containment =
          el.opt.containment == "body" ? window : el.opt.containment;
        var aspectRatio = w / h;
        ...
        if (!el.opt.preserveTop)
          image.css(
            "margin-top",
            ($(containment).height() - image.height()) / 2
          );
    ...
    changePhoto: function(url, el) {
      ...
      if (el.opt.onChange) el.opt.onChange(el.opt, el.opt.imageCounter);
      ...
      var image = $("<img/>")
        ...
        .load(function() {
          ...
          el.opt.effect =
            typeof el.opt.effect == "object"
              ? el.opt.effect
              : $.mbBgndGallery.effects[el.opt.effect];

          $("#bgndGallery_" + el.opt.galleryID + " img").CSSAnimate(
            el.opt.effect.exit,
            el.opt.effTimer,
            0,
            el.opt.effect.exitTiming,
            function(el) {
              el.remove();
            }
          );
    ...
    play: function(el) {
      ...
      var imgToRemove = $("#bgndGallery_" + el.opt.galleryID + " img").not(
        ":last"
      );
      imgToRemove.remove();
      ...
      if (el.opt.onPlay) el.opt.onPlay(el.opt);

      el.opt.changing = setTimeout(function() {
        if (el.opt.paused) return;

        if (el.opt.onNext) el.opt.onNext(el.opt);
        ...
        $.mbBgndGallery.changePhoto(
          el.opt.images[el.opt.imageCounter],
          $(el.opt.containment).get(0)
        );
    ...
    pause: function(el) {
      ...
      if (el.opt.onPause) el.opt.onPause(el.opt);
    },
    ...
    prev: function(el) {
      ...
      if (el.opt.onPrev) el.opt.onPrev(el.opt);

      $.mbBgndGallery.pause(el);

      clearTimeout(el.opt.changing);
      if (el.opt.imageCounter == 0) el.opt.imageCounter = el.opt.images.length;
    ...
    buildControls: function(controls, el) {
      ...
      if (el.opt.activateKeyboard) $.mbBgndGallery.keyboard(el);
    },
    ...
    runFullscreen: function(gallery, el) {
      ...
      function launchFullscreen(element) {
        ...
        setTimeout(function() {
          var fullscreenchange = $.browser.mozilla
            ? "mozfullscreenchange"
            : $.browser.webkit
      ...
      if (el.isFullscreen) {
        ...
        var controls = $(el.opt.controls)
          .clone(true)
          .addClass("fullScreen_controls")
          .css({ position: "absolute", zIndex: 1000, bottom: 20, right: 20 });
